...
# This code is for saving current values from transient run
...
QC <- read.csv("simu_Cpools_lastyear_cnp.csv", header = FALSE)
reQC <- colMeans(QC)
reQC
...
recflux <- cflux[, c(1:4, 8)] # GPP,ER,NEE,LAI,Rh. # clean data - 1:5
colSums(recflux)
mean(recflux[, 5]) # Rh
sd(recflux[, 5]) # Rh
mean(recflux[, 4]) # LAI
...
##### CN #####
setwd("your_path")
QC <- read.csv("simu_Cpools_lastyear_cn.csv", header = FALSE)
reQC <- colMeans(QC)
reQC
...
##### C #####
setwd("your_path")
QC <- read.csv("simu_Cpools_lastyear_c.csv", header = FALSE)
reQC <- colMeans(QC)
reQC
